...
public class SkillUtil {
    ...
    public static int getTimeBroly(int level) { //thời gian tồn tại biến broly
        return (level + 5) * 10000;
    }
public static int getTimetienhoa1(int level) { //thời gian tồn tại tiến hóa
        return (level + 5) * 5000;
    }
public static int getTimetienhoa(boolean lastLevel, int coolDown) { //thời gian biến hình
        int per = lastLevel ? 60 : 120;
        return coolDown * per / 100;
    }
 public static int getTimeVohieu(int level) { //thời gian tồn tại vô hiệu
...
//        return 10000;
    ...
    public static byte getTempSkillSkillByItemID(int id) {
        if (id >= 66 && id <= 72) {
            ...
            return Skill.HUYT_SAO; // id skill temp
          } else if (id >= 1844 && id <= 1848) {
            return Skill.TIEN_HOA;
        }else if (id >= 1849 && id <= 1853) {
            return Skill.TIEN_HOA;
        }else if (id >= 1854 && id <= 1858) {
            return Skill.TIEN_HOA;
        }
        else {
            return -1;
    ...
    public static Skill getSkillByItemID(Player pl, int tempId) {
        if (tempId >= 66 && tempId <= 72) {
            ...
            return getSkillbyId(pl, Skill.HUYT_SAO);
                  }else if (tempId >= 1844 && tempId <= 1848) {
            return getSkillbyId(pl, Skill.TIEN_HOA);
        }else if (tempId >= 1849 && tempId <= 1853) {
            return getSkillbyId(pl, Skill.TIEN_HOA);
        }else if (tempId >= 1854 && tempId <= 1858) {
            return getSkillbyId(pl, Skill.TIEN_HOA);
        } else {
            return null;
        }
