...
public class Manager {
    ...
    public static final String queryTopSM = "SELECT id, CAST(SUBSTRING_INDEX(data_point, ',', 2) AS UNSIGNED) AS sm FROM player ORDER BY sm DESC LIMIT 10;";
    ...
    public static List<TOP> topSieuHang1;
...
// ...
    ...
    private void loadDatabase() {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ...
            ps = con.prepareStatement("select * from head_avatar_2_frames");
            rs = ps.executeQuery();
            while (rs.next()) {
            ...
            for (int i = 0; i < ITEM_TEMPLATES.size(); i++) {
                if (ITEM_TEMPLATES.get(i).type == 5 && ITEM_TEMPLATES.get(i).id >= 598 && ITEM_TEMPLATES.get(i).id <= 609) {
...
//                RUBY_REWARDS.add(Util.sendDo(861, 0, new ArrayList<>()));
            ...
            Logger.log(Logger.GREEN, "Finish load item ky gui [" + ShopKyGuiManager.gI().listItem.size() + "]!\n");
            ...
            Logger.log(Logger.YELLOW, "Load top sm thành công (" + topSM.size() + ")\n");
            topNV = realTop(queryTopNV, con);
            Logger.log(Logger.YELLOW, "Load top nv thành công (" + topNV.size() + ")\n");
            topSD = realTop(queryTopSD, con);
            Logger.log(Logger.YELLOW, "Load top Sức đánh thành công (" + topSD.size() + ")\n");
            topSK = realTop(queryTopSK, con);
            Logger.log(Logger.YELLOW, "Load top sk thành công (" + topSK.size() + ")\n");
            topPVP = realTop(queryTopPVP, con);
            Logger.log(Logger.YELLOW, "Load top pvp thành công (" + topSK.size() + ")\n");
            topCS = realTop(queryTopCS, con);
            Logger.log(Logger.YELLOW, "Load top chuyen Sinh thành công (" + topCS.size() + ")\n");
            topNHS = realTop(queryTopNHS, con);
            Logger.log(Logger.YELLOW, "Load Top Ngũ Hành Sơn Thành Công (" + topNHS.size() + ")\n");
            topNap = realTop(queryTopNap, con);
            Logger.log(Logger.YELLOW, "Load top nap thành công (" + topNap.size() + ")\n");
            TopGapThu = realTop(queryTopGapThu, con);
            Logger.log(Logger.YELLOW, "Load Top Gắp Thú Thành Công (" + TopGapThu.size() + ")\n");
            TopCauCa = realTop(queryTopCauCa, con);
            Logger.log(Logger.YELLOW, "Load Top Câu Cá Thành Công (" + TopCauCa.size() + ")\n");
            TopLeoThap = realTop(queryTopLeoThap, con);
            Logger.log(Logger.YELLOW, "Load top Leo Tháp thành công (" + TopLeoThap.size() + ")\n");
            topSieuHang = realTopSieuHang(con);
            Logger.log(Logger.YELLOW, "Load top Siêu hạng thành công (" + topSieuHang.size() + ")\n");
            TopThachDau = realTop(queryTopThachDau,con);
            Logger.log(Logger.YELLOW, "Load top Thách Đấu Whis thành công (" + TopThachDau.size() + ")\n");
            topSieuHang1 = realTopSieuHang2(con);
            Logger.log(Logger.YELLOW,"Load top Siêu hạng thành công (" + topSieuHang1.size() + ")\n");
    ...
    public void loadAllTop() {
        try (Connection con = GirlkunDB.getConnection()) {
            topSM = realTop(queryTopSM, con);
            topSD = realTop(queryTopHP, con);
            topHP = realTop(queryTopSD, con);
            topKI = realTop(queryTopKI, con);
            topNV = realTop(queryTopNV, con);
            topSK = realTop(queryTopSK, con);
            topPVP = realTop(queryTopPVP, con);
            topNHS = realTop(queryTopNHS, con);
            TopGapThu = realTop(queryTopSM, con);
            TopCauCa = realTop(queryTopHP, con);
            TopLeoThap = realTop(queryTopSD, con);
            topSieuHang = realTop(queryTopKI, con);
            TopThachDau = realTop(queryTopNV, con);
        } catch (Exception e) {
        }
    }
    ...
    public static List<TOP> realTop(String query, Connection con) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                switch (query) {
                    case queryTopSM:
                        top.setInfo2("Bảng Xếp Hạng SỨC MẠNH");
                    ...
                    case queryTopNV:
                        ...
                        break;
                    case queryTopSK:
                        top.setInfo2("Bảng Xếp Hạng ĐIỂM");
                    ...
                    case queryTopPVP:
                        ...
                        break;
                    case queryTopNHS:
                        top.setInfo2("Bảng Xếp Hạng NHS");
                        top.setInfo1(rs.getInt("NguHanhSonPoin") + " điểm");
                        break;
                    case queryTopNap:
                        top.setInfo2("Bảng Xếp Hạng NẠP");
                        top.setInfo1(rs.getInt("tongnap") + " VND");
                        break;
                    case queryTopCS:
                        top.setInfo2("Bảng Xếp Hạng CHUYỂN SINH");
                    ...
                    case queryTopLeoThap:
                        top.setInfo2("Bảng Xếp Hạng LEO THÁP");
                        top.setInfo1(rs.getInt("leothap") + " tầng");
                        break;
                    case queryTopThachDau:
                        top.setInfo2("Bảng Xếp Hạng THÁCH ĐẤU");
                        top.setInfo1(rs.getInt("thachdauwhis") + " LEVEL");
                        break;
                    case queryTopCauCa:
                        top.setInfo2("Bảng Xếp Hạng ĐIỂM CÂU CÁ");
                        top.setInfo1(rs.getInt("cauca") + " điểm");
                        break;
                    case queryTopSD:
                        top.setInfo2("Bảng Xếp Hạng SỨC ĐÁNH");
    ...
    public void loadProperties() throws IOException {
        ...
        Object value = null;
        //###Config sv
        if ((value = properties.get("server99.obito.port")) != null) {
            ServerManager.PORT = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server99.obito.name")) != null) {
            ServerManager.NAME = String.valueOf(value);
        }
        if ((value = properties.get("server99.obito.sv")) != null) {
            SERVER = Byte.parseByte(String.valueOf(value));
        }
        String linkServer = "";
        for (int i = 1; i <= 10; i++) {
            value = properties.get("server99.obito.sv" + i);
            if (value != null) {
                linkServer += String.valueOf(value) + ":0,";
            }
        }
        DataGame.LINK_IP_PORT = linkServer.substring(0, linkServer.length() - 1);
        if ((value = properties.get("server99.obito.waitlogin")) != null) {
            SECOND_WAIT_LOGIN = Byte.parseByte(String.valueOf(value));
        }
        if ((value = properties.get("server99.obito.maxperip")) != null) {
            MAX_PER_IP = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server99.obito.maxplayer")) != null) {
            MAX_PLAYER = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server99.obito.expserver")) != null) {
            RATE_EXP_SERVER = Byte.parseByte(String.valueOf(value));
        }
        if ((value = properties.get("server99.obito.local")) != null) {
