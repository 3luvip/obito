...
//    public void setIstienhoa1(Player player) {
//        try {
//            Thread.sleep(1000);
//        } catch (InterruptedException ex) {
//            Logger.getLogger(EffectSkillService.class.getName()).log(Level.SEVERE, null, ex);
...
//    }
    public void setIstienhoa(Player player) {
        ...
        boolean lastLevel = player.effectSkill.leveltienhoa >= skillLevel - 1;

        player.effectSkill.istienhoa = true;
        player.effectSkill.leveltienhoa = Math.min(skillLevel, player.effectSkill.leveltienhoa + 1);
        player.effectSkill.timetienhoa = SkillUtil.getTimetienhoa(lastLevel, template.coolDown);
        player.effectSkill.lastTimeUptienhoa = System.currentTimeMillis();

        if (!lastLevel && template.coolDown > 0) {
            player.playerSkill.skillSelect.coolDown = template.coolDown * 1 / 800;
        } else {
